...
export const accountsController = {
  ...
  signup: {
    ...
    handler: async function (request, h) {
      ...
      /// Sanitized
      user.firstName = clean(user.firstName);
      user.lastName = clean(user.lastName);
  ...
  login: {
    ...
    handler: async function (request, h) {
      // Sanitize login
      const email = clean(request.payload.email);
      const password = clean(request.payload.password);
      ...
      /// Bcrypt hash check
      const passwordMatches = await bcryptUtils.checkPassword(password, user.password);
      if (!passwordMatches) {
        return h.view("login-error");
      }
      request.cookieAuth.set({ id: user._id });
